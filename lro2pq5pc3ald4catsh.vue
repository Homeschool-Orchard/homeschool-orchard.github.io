<template>   <div>     <a :id="`basic-contact-form-${index}`"></a>     <div :class="colors.background" class="relative isolate px-6 py-24 sm:py-32 lg:px-8">
<div class="mx-auto max-w-2xl text-center">
 ...
 <form @submit.prevent="handleSubmit" class="mx-auto mt-16 max-w-xl sm:mt-20">
  <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
    <div class="sm:col-span-2">
      <label for="name" :class="colors.text" class="block text-sm font-semibold leading-6"> asfdaf Name <span class="text-red-600">*</span></label>
      <div class="mt-2.5">
 ...
 <input v-model="formData.email" type="email" required name="email" autocomplete="email" class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6">
    ...
    <div class="sm:col-span-2">
      <label for="phone-number" :class="colors.text" class="block text-sm font-semibold leading-6">Phone number</label>
      <div class="mt-2.5">

 <input v-model="formData.phone" type="tel" name="phone-number" autocomplete="tel" class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6">
      </div>
    </div>      </div>
  </div>
   <div v-if="errors.length" class="rounded-md bg-red-50 p-4 mx-auto max-w-xl mt-10">
    <div class="flex">
...
<path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
   ...
   <div v-else-if="success" class="rounded-md bg-green-50 p-4 mx-auto max-w-xl mt-10">
    <div class="flex">
      <div class="flex-shrink-0">
...
<path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
    ...
    <div class="mt-10">
    <input type="hidden" name="hidden" v-model="hiddenValue">
    <button
 ...
 {'color': settings.button_text_color}
      ...
      v-text="settings.button_text"
    />
  </div>
</form>     </div>   </div> </template><script> export default {   props: {     index: {
type: Number,
default: 0     },     schema: {
type: Object,
default: () => ({})     },     site: {
...
const data = {};
 this.schema.settings?.forEach((setting) => {
  data[setting.id] = setting.value;
});
 return data;     },      colors() {
return themeColors(this.settings.theme);     }   },    methods: {     async handleSubmit() {
if (this.hiddenValue) {
  this.errors.push('Failed to submit form. Please try again or contact the site administrator.');
  return;
}
...
this.success = false;     }   } } </script><style></style>